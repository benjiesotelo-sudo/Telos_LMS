...
- **Grade Editor (bottom, editable):** a full grid **identical in columns** to the sheet, but assessment cells are **click-to-edit `raw / total`** (prefilled with current effective score: override → auto → blank). **MG/FG/Course Mark/LG recompute LIVE** as you type (preview before saving). Edited-but-unsaved cells get a blue outline; one **Save changes (N edited)** button batches all edits across assessments; a **Discard** link resets. **Safe revert:** online item → one-click `↺ auto N`; manual item → `Clear` (red) with a single **Save-time confirm** before erasing a manual-only score.
