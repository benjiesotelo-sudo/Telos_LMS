...
## 🔓 Reveal-answers rule changed (2026-07-01) — SHIPPED
Answers now reveal to students **as soon as there's NO Closes time set** (previously: no close = answers NEVER revealed — a silent trap). A **future** Closes time still holds answers back until it passes; a past close (or no close) reveals immediately. **ALL four types (quiz/exam/homework/activity) now behave identically** — the old "activity always reveals regardless of close" carve-out was removed (2026-07-01, per Benjie: the reveal toggle + ownership + graded + server-only key are enough safeguards). Gate is purely `closes_at == null || closes_at <= now` (no type branch) in `getRevealedAnswers.ts` + `getStudentData.ts`; the instructor note in `AssignmentMetaControls.tsx` is informational for any type (a future close *delays* reveal; clear it to reveal on submit). 303 unit tests green; reveal flow verified in a real browser. **Heads-up:** for any assessment taken at staggered times, set a Closes time so early finishers can't leak answers to classmates.

## 🔬 Browser audit harness (2026-07-01)
- `e2e/audit.mjs` — broad page+interaction smoke (30 checks). `e2e/audit-full.mjs` — TRUE end-to-end: builds course → class → import → assign → reveal → enroll link → register student → approve → student takes BOTH assessments → answers → review → instructor grade sheet (22 checks, all pass).
- Run: `supabase db reset` → `node e2e/seed.mjs` → local-pointed `:3100` prod server (`NEXT_PUBLIC_SUPABASE_URL=http://127.0.0.1:54321` + local anon/service keys + `NEXT_PUBLIC_SITE_URL=http://localhost:3100` + `PORT=3100`, `npm run build` then `npm run start`) → `node e2e/audit-full.mjs`. Screenshots → `e2e/shots/` (gitignored). Reports: `e2e/AUDIT_*_REPORT.md`. `FEATURES.md` = the feature checklist/tracker.
- **Observation (possible follow-up):** toggling **Reveal Answers** on TWO assignments in quick succession can drop one save — `setAssignmentMeta` calls `refresh()`, and the re-render races the second toggle. Human-paced clicking is unlikely to hit it; a small await-before-next-toggle (or debounce) would harden it.
