...
Quiz/exam/homework now reveal answers to students **as soon as there's NO Closes time set** (previously: no close = answers NEVER revealed — a silent trap). A **future** Closes time still holds answers back until it passes; a past close (or no close) reveals immediately; **activity** reveals immediately as before. Gate is now `closes_at == null || closes_at <= now` in `getRevealedAnswers.ts` + `getStudentData.ts`; the instructor note in `AssignmentMetaControls.tsx` is now informational (a future close *delays* reveal; clear it to reveal on submit). 303 unit tests green; verified in a real browser (quiz with reveal-on + no close shows answer review). **Heads-up:** for a real exam taken at staggered times, set a Closes time so early finishers can't leak answers to classmates.
